...
class Smarty_Compiler extends Smarty{
  ...
  function _compile_file($resource_name, $source_content, &$compiled_content)
  {
    ...
    $compiled_content = str_replace('<?', "<?php echo '<?php' ?>\n", $compiled_content);
  ...
  function _parse_var_props($val)
  {
    ...
    }elseif (preg_match('~^' . $this->_const_regexp . '(?:' . $this->_mod_regexp . '*)$~', $val)){
      // config var
      return $this->_parse_const_var($val);
   ...
   * @param string $conf_var_expr
   */
  function _parse_const_var($conf_var_expr)
  {
  ...
  function _push_cacheable_state($type, $name)
  {
    ...
    if ($_cacheable || 0 < $this->_cacheable_state++) return '';
  ...
  {
    $_cacheable = !isset($this->_plugins[$type][$name]) || $this->_plugins[$type][$name][4];
    if ($_cacheable || --$this->_cacheable_state > 0) return '';
