...
class router {
  ...
  static function draw($action, $module = 'web', $isAjax= false) {
			if ($action { 0 } == '_') {
      ...
      if (strpos('/',$module) === false) {
        ...
        $controller = substr($module,strpos('/',$module) );
        $module = substr($module,0, strpos('/',$module) );
      }
  		$classname = 'Control_'.ucfirst($controller);

      require_once ( dirname(dirname(__FILE__)).'\config\init_'.$module.'.php');
      include (INC.'controller'.DS.'control.'.$controller.'.php');
