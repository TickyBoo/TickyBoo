...
// *****************************************************************************
// ***** Toute remarque, tout commentaire, tout rapport de bug, toute recompence
...
class pdf_barcode
{
    ...
    */
    ...
    var $C39 =  array(
                ...
                '9' => "101100101101",  'A' => "101100101101",  'B' => "101101001011",
    ...
    * return            void
    */
    function pdf_barcode($TYPE, $CODE, $HEIGHT=10, $HR="Y", $WIDTH=0, $SHOWTYPE="N")
    {
        if(!$this->check($TYPE,$CODE)){$this->ERR=1;return;}        
    ...
    */
	...
	$nb_elem = strlen($this->strCode);
        ...
        $color[0] = array( 255,255,255);
        $color[1] = array( 0,0,0);
        $color[2] = array( 160,160,160);
        
        //$this->filledrectangle($pdf, 0, 0, $nb_elem+20, $this->HEIGHT, $color[0]);
        ...
        for($i=0;$i<$nb_elem;$i++)
        {
			...
			if($this->TYPE == "POSTNET") {
			    if($fill_color == "1") $this->filledrectangle($pdf, $posX, ($posY+1), $posX+1, ($posY+20), $color[1]);
			    else $this->filledrectangle($pdf, $posX, ($posY+12), $posX+1, ($posY+20), $color[1]);
				$intL = 4 ;
			} elseif($this->TYPE == "KIX") {
			    if($fill_color == "0") $this->filledrectangle($pdf, $posX, ($posY+1), $posX+1, ($posY+13), $color[1]);
			    elseif($fill_color == "1") $this->filledrectangle($pdf, $posX, ($posY+7), $posX+1, ($posY+19), $color[1]);
			    elseif($fill_color == "2") $this->filledrectangle($pdf, $posX, ($posY+7), $posX+1, ($posY+13), $color[1]);
			    else $this->filledrectangle($pdf, $posX, ($posY+1), $posX+1, ($posY+19), $color[1]);
				$intL = 4 ;
                ...
                for( $j=0; $j<sizeof($coord); $j++) {
                	...
                	$this->filledrectangle($pdf, $X1, $Y1, $X2, $Y2, $color[1]);
                }
                $intL = 24 ;
            } 
    		else  {
			    if($fill_color == "1") $this->filledrectangle($pdf, $posX, $posY, $posX+1, ($posY+$intH), $color[1]);
			}
//            $this->filledrectangle($pdf, $posX, $posY, $posX, ($posY+$intH), $color[$fill_color]);
...
//        return $img;
    ...
    function filledrectangle (&$pdf,$x0,$y0,$x1,$y1,$color){
      $pdf->setColor($color[0],$color[1],$color[2]);
      $pdf->setStrokeColor($color[0],$color[1],$color[2]);

      //echo $x0." ".$x1." <br>";
      $pdf->filledrectangle($x0,$y0,$x1-$x0,$y1-$y0);
    } 


function check(&$type,&$code){
...
/*
if($width >= 10) $hw = $width;
if($height > 0) $hh = $height;
if($readable == "Y") $hr = $readable;
