...
class RepublishEventView extends AdminView {
    ...
    function confirm_button (&$event, $show_button = true)
    {
        ...
        if ($show_button) {
            ...
            echo "
                  <input type='hidden' name='action' value='republish'><br>
        ...
        if ($_POST['confirm'] == 'YES' and $_POST['event_id'] > 0) {
            if ($event = Event::load($_POST['event_id'], false) and $event->event_organizer_id == $_SHOP->organizer_id) {
                if ($event->restart_sales()) {
                    ...
                    echo "<div class='err'> <b>'{$event->event_name}'</b> " . restart_failure . "</div>\n";
                }
                ...
                if ($event->event_rep == 'main' and $_POST['also_sub'] and $subs = Event::load_all_sub($event->event_id)) {
                    foreach($subs as $sub) {
                        ...
                        if ($sub->event_status == 'nosal') {
                            if ($sub->restart_sales()) {
                                echo "<div class='success'> <b>'$date'</b> " . restart_success . "</div>\n";
                            } else {
                                echo "<div class='err'> <b>'$date'</b> " . restart_failure . "</div>\n";
                            }
                        }
                    }
                }
