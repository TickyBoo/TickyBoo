...
class EventPropsView extends EventViewCommon {
	...
	function fill_images() {
		$img_pub['pub'] = array( "src" => 'images/grun.png', "title" =>
			"Click to Unpublish.", "alt" => "Published", "link" =>
			"view_event.php?action=unpublish&event_id=" );

		$img_pub['unpub'] = array( "src" => 'images/rot.png', 'title' =>
			"Click to publish.", 'alt' => "New event", 'link' =>
			"view_event.php?action=publish&event_id=" );

		$img_pub['nosal'] = array( "src" => 'images/grey.png', "title" =>
			"Click to republish.", "alt" => "Unpublished", "link" =>
			"view_event.php?action=republish&event_id=" );
	...
	function event_sub_list( $event_main_id, &$alt ) {
		...
		while ( $row = shopDB::fetch_assoc($res) ) {
			...
			echo "<tr class='admin_list_row_$alt'>
                ...
                <td class='admin_list_item'>" . showstr( $row['ort_name'] ) .
				"</td>\n";
	...
	function event_list() {
		...
		while ( $row = shopDB::fetch_assoc($res) ) {
            ...
            <td width=18 class='admin_list_item'><a class='link' alt='{$img_pub[$row['event_status']]['alt']}'
               ...
               <img border='0' alt='{$img_pub[$row['event_status']]['alt']}'
                               title='{$img_pub[$row['event_status']]['title']}'
                               src='{$img_pub[$row['event_status']]['src']}'></a></td>
            <td class='admin_list_item'>" . showstr( $row['event_name'], 30 ) .
				"</td>\n";

			if ( $row['event_rep'] == 'main' ) {
				echo "<td colspan=2 class='admin_list_item'><a class='link' title='" . add .
					"' alt='" . add . "' href='view_event.php?action=add_sub&event_main_id={$row['event_id']}'>" .
					multi . "</a></td>\n";
			} else {
			...
			echo "<td class='admin_list_item' nowrap='nowrap'>
                  <a class='link' href='view_event.php?action=edit&event_id={$row['event_id']}'><img src='images/edit.gif' border='0' alt='" .
				edit . "' title='" . edit . "'></a>\n";

			if ( $row['event_pm_id'] ) {
				echo "<a class='link' href='{$_SERVER['PHP_SELF']}?action=view_pm&pm_id={$row['event_pm_id']}'><img src='images/pm.png' border='0' alt='" .
					place_map . "' title='" . place_map . "'></a>\n";
			}
			...
			if ( ($row['event_pm_id'] and $row['event_status'] == 'unpub') or (!$row['event_pm_id'] and
				...
				echo "<a class='link' target='_blank' href='archive_event.php?event_id={$row['event_id']}'>
                         <img src='images/archive.png' border='0' alt='" .
					Archive . "' title='" . Archive . "'></a>\n";
				echo "<a class='link' title='" . delete_item . "' href='javascript:if(confirm(\"" .
					delete_item . "\")){location.href=\"view_event.php?action=remove&event_id={$row['event_id']}\";}'><img src='images/trash.png' border='0' alt='" .
		...
		echo "</table>\n";
		echo "<br><center><a class='link' href='view_event.php?action=add'>" . add .
			"</a></center>";
	}
	// #######################################################
	// #######################################################
	...
	function draw() {
		...
		} elseif ( $_REQUEST['action'] == 'republish' ) {
	...
	function event_check( &$data, &$err ) {
		...
		if ( (isset($data['event_time-h']) and strlen($data['event_time-h']) > 0) or (isset
			($data['event_time-m']) and strlen($data['event_time-m']) > 0) ) {
			$h = $data['event_time-h'];
			$m = $data['event_time-m'];
			if ( !is_numeric($h) or $h < 0 or $h > 23 ) {
				$err['event_time'] = invalid;
			} else
				if ( !is_numeric($m) or $h < 0 or $m > 59 ) {
					$err['event_time'] = invalid;
				} else {
					$data['event_time'] = "$h:$m";
				}
		...
		if ( (isset($data['event_open-h']) and strlen($data['event_open-h']) > 0) or (isset
			($data['event_open-m']) and strlen($data['event_open-m']) > 0) ) {
			$h = $data['event_open-h'];
			$m = $data['event_open-m'];
			if ( !is_numeric($h) or $h < 0 or $h > 23 ) {
		...
		if ( (isset($data['event_end-h']) and strlen($data['event_end-h']) > 0) or (isset
			($data['event_end-m']) and strlen($data['event_end-m']) > 0) ) {
			$h = $data['event_end-h'];
			$m = $data['event_end-m'];
			if ( !is_numeric($h) or $h < 0 or $h > 23 ) {
				$err['event_end'] = invalid;
			} else
				if ( !is_numeric($m) or $h < 0 or $m > 59 ) {
					$err['event_end'] = invalid;
				} else {
		...
		if ( (isset($data['event_date-y']) and strlen($data['event_date-y']) > 0) or (isset
			($data['event_date-m']) and strlen($data['event_date-m']) > 0) or (isset($data['event_date-d']) and
			strlen($data['event_date-d']) > 0) ) {
			$y = $data['event_date-y'];
			$m = $data['event_date-m'];
			$d = $data['event_date-d'];
