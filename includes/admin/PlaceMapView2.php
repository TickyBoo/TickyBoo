...
class PlaceMapView2 extends AdminView {
    ...
    function pm_form (&$data, &$err, $title)
    {
        ...
        if ($event) {
           ...
           $dist_view->discount_list($event->event_id, false, $live);
...
*/
    ...
    function pm_list ($ort_id)
    {
        ...
        if ($mine) {
            echo "<tr><td colspan=6 align=center><a class='link' href='{$_SERVER['PHP_SELF']}?action=add_pm&pm_ort_id=$ort_id'>" . add . "</a></td></tr>";
        }
    ...
    <br>
    <center><a class=link href='{$_SERVER['PHP_SELF']}?action=view_pm&pm_id=$pm_id'>" . place_map . "</a></center>
    ";
    }
    ...
    function draw ()
    {
        ...
        } else if ($_GET['action'] == 'add_pm') {
            $this->pm_form($_GET, $err, '');

        } else if ($_POST['action'] == 'insert_pm') {
        ...
        } else if ($_GET['action'] == 'copy_pm' and $_GET['pm_id'] > 0) {
