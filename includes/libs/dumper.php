...
// Path and URL of the backup files
define('PATH', 'backup/');
define('URL',  'backup/');
// The maximum run time of the script in seconds
// 0 - no limit
...
// Databases, if the server does not allow the list of databases,
// and nothing is displayed after authentication. List the names, separated by commas
define('DBNAMES', '');
...
// When you add a 'forced->', for instance 'forced-> cp1251', encoding tables when the reconstruction will be forcibly replaced by cp1251
// Can also specify the appropriate comparison for example 'cp1251_ukrainian_ci' or 'forced-> cp1251_ukrainian_ci'
...
class dumper {
	...
	function backup() {
		...
		echo tpl_page(tpl_process("Creating Backup of DB"), $buttons);
		...
		if (!$db) {
			echo tpl_l("WARNING! No select database!", C_ERROR);
			echo tpl_enableBack();
		    exit;
		}
		...
		echo tpl_l("Creating file with the backup database:<BR>\\n  -  {$this->filename}");
        ...
        foreach ($tables AS $table){
			...
			if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
				if (CHARSET == 'auto') {
					...
					echo tpl_l("Established the connection charset `" . $tab_charset[$table] . "`.", C_WARNING);
					$last_charset = $tab_charset[$table];
				}
				else{
					echo tpl_l('Charset of connection and table charset does not match:', C_ERROR);
					echo tpl_l('Table `'. $table .'` -> ' . $tab_charset[$table] . ' (connection '  . CHARSET . ')', C_ERROR);
				}
			}
			echo tpl_l("Proccessing Table `{$table}` [" . fn_int($tabinfo[$table]) . "].");
        	// Table creation
        ...
        echo tpl_l(str_repeat("-", 60));
        $this->fn_close($fp);
		echo tpl_l("Backup of DB `{$db}` is created.", C_RESULT);
		echo tpl_l("DB size:       " . round($this->size / 1048576, 2) . " Mb", C_RESULT);
		...
		// Sending data to the global statistics (Niels, we no need this)
	...
	function restore(){
		...
		echo tpl_page(tpl_process("Restore DB from Backup"), $buttons);
		...
		if (!$db) {
			echo tpl_l("WARNING! No selected DB!", C_ERROR);
			echo tpl_enableBack();
		    exit;
		}
		echo tpl_l("Connecting to DB `{$db}`.");
		...
		if(preg_match("/^(.+?)\.sql(\.(bz2|gz))?$/", $file, $matches)) {
			...
			if (!file_exists(PATH . "/{$file}")) {
    		    echo tpl_l("ATTENTION! File Not Found!", C_ERROR);
				echo tpl_enableBack();
    		    exit;
    		}
			...
			echo tpl_l("WARNING! Do not select a file!", C_ERROR);
			echo tpl_enableBack();
		    exit;
		}
		echo tpl_l(str_repeat("-", 60));
		...
		// Establishing charset of the connection
		if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // Encryption by default if not specified in the dump encoding
			...
			echo tpl_l("Established charset of the connection `" . $this->restore_charset . "`.", C_WARNING);
			    ...
			    if (preg_match("/;$/", $str)) {
					...
					if (empty($insert)) {
						...
						elseif (preg_match("/CREATE TABLE/i", $sql)){
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										...
										$cache .= tpl_l("Established charset of the connection `" . $charset[2] . "`.", C_WARNING);
										$last_charset = $charset[2];
									}
									else{
										$cache .= tpl_l('Charset of connection and the table charset does not match:', C_ERROR);
										$cache .= tpl_l('Table `'. $table .'` -> ' . $charset[2] . ' (connection '  . $this->restore_charset . ')', C_ERROR);
									}
								}
								// Change the encoding (charset) if the encoding specified force
							...
							elseif(CHARSET == 'auto'){ // Insert the encoding for the tables if it is not listed, and an auto coding
								...
								if ($this->restore_charset != $last_charset) {
									...
									$cache .= tpl_l("Established charset of the connection `" . $this->restore_charset . "`.", C_WARNING);
									$last_charset = $this->restore_charset;
								}
							}
						}
		...
		if (isset($info[3])) echo tpl_l("Creation Date of Backup: {$info[3]}", C_RESULT);
		echo tpl_l("Queries to the database: {$q}", C_RESULT);
		...
		// Sending data to the global statistics
	...
	function main(){
		$this->comp_levels = array('9' => '9 (maximum)', '8' => '8', '7' => '7', '6' => '6', '5' => '5 (average)', '4' => '4', '3' => '3', '2' => '2', '1' => '1 (minimum)','0' => 'Without compression');
		...
		$buttons = "<INPUT TYPE=submit VALUE=Apply><INPUT TYPE=button VALUE=Exit onClick=\"location.href = 'dumper.php?reload'\">";
...
<HTML>
<HEAD>
<TITLE>FusionTicket Dumper</TITLE>
...
<B><A HREF=http://sypex.net/products/dumper/ STYLE="color: white; text-decoration: none;">FusionTicket Dumper</A></B><IMG ID=GS WIDTH=1 HEIGHT=1 STYLE="visibility: hidden;"></TD>
</TR>
<TR>
...
Backup / Creating DB Backup&nbsp;</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD WIDTH=35%>DB:</TD>
...
Restore / Restoring DB from Backup&nbsp;</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>DB:</TD>
...
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>For FusionTicket Dumper requires:<BR> - Internet Explorer 5.5+, Mozilla or Opera 8+ (<SPAN ID=sie>-</SPAN>)<BR> - enable to run execution JavaScript (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
...
return <<<HTML
<CENTER>
<H1>You have no permission to view this directory</H1>
</CENTER>
...
function SXD_errorHandler($errno, $errmsg, $filename, $linenum, $vars) {
	...
	echo tpl_l("{$dt}<BR><B>We have some Error!</B>", C_ERROR);
