...
<table cellpadding="0" CELLSPACING=0 border=0 width='100%'>
  <tr>
  	...
  	<td valign='top' align="right">
     	<form action='kasse.php' method='post' name='handling' onSubmit='this.submit.disabled=true;return true;'>
      		...
      		{handling www='on' event_date=$min_date }
        		<tr class="{cycle name='payments' values="TblHigher,TblLower"}">
        		  <td class='payment_form'>
          		  <input checked="checked" type='radio' id='{$shop_handling.handling_id}_check' class='checkbox_dark' name='handling' value='{$shop_handling.handling_id}'>
        		  </td>
        		  <td class='payment_form'>
        		  ...
        		  <td class='payment_form'>
           			{assign var=fee value="`$total*$shop_handling.handling_fee_percent/100.00+$shop_handling.handling_fee_fix`"} + {valuta value=$fee|string_format:"%.2f"}
        		  </td>
        		</tr>
          {/handling}
